...
describe("tradeFilterSummary", () => {
  ...
  test("a bay's `only` reads in both directions", () => {
    // It is the one place the claim is legible while the search is collapsed,
    // and it has to be true from either end: on jkap's own bay it is what he
    // got, on the bay opposite him it is what he gave — the same claim about
    // the same side, said from the two ends `sideLabel` already speaks from.
    assert.equal(
      tradeFilterSummary(
        filters({
          sides: [side({ manager: "u1" }), side({ players: ["p1"], only: true })],
        }),
        NAMES,
      ),
      "jkap gave only Malik Nabers",
    );
    assert.equal(
      tradeFilterSummary(
        filters({
          sides: [side({ manager: "u1", players: ["p1"], only: true }), EMPTY_SIDE],
        }),
        NAMES,
      ),
      "jkap got only Malik Nabers",
    );
    assert.equal(
      tradeFilterSummary(
        filters({
          sides: [
            ...
            side({ picks: ["2027-1"], only: true }),
          ],
        }),
        NAMES,
      ),
      "one side got only Malik Nabers · the other side got only 2027 1st",
    );
  });
...
describe("the side mutations", () => {
  ...
  test("swapping is the two bays, exchanged", () => {
    ...
    // Which carries the claim with its bay — a flag left behind would be the
    // reader's question asked of the other side of the trade.
    assert.equal(swapped.sides[0].only, true);
    assert.equal(swapped.sides[1].only, false);
  });

  test("`only` is a claim about one bay and leaves the other alone", () => {
    // Unlike an asset and a manager, which move: two bays both claiming
    // exhaustiveness is the perfectly good question "was this trade exactly
    // these for those".
    const set = filters({
      sides: [side({ players: ["p1"] }), side({ players: ["p2"] })],
    });
    const left = toggleSideOnly(set, 0);
    assert.equal(left.sides[0].only, true);
    assert.equal(left.sides[1].only, false);

    const both = toggleSideOnly(left, 1);
    assert.equal(both.sides[0].only, true);
    assert.equal(both.sides[1].only, true);

    // And it toggles back off in its own bay.
    assert.equal(toggleSideOnly(both, 0).sides[0].only, false);
  });

  test("the neutral bay makes no claim", () => {
    assert.equal(EMPTY_SIDE.only, false);
  });
});
...
describe("sideSelectionCount", () => {
  ...
  test("`only` is not one of them, and cannot be", () => {
    // It is a modifier over things already in this number rather than a thing
    // of its own, and a bay can never carry it without carrying the asset that
    // made it pressable — so counting it would report two narrowings where a
    // reader named one player.
    const named = filters({ sides: [side({ players: ["a"] }), EMPTY_SIDE] });
    assert.equal(sideSelectionCount(toggleSideOnly(named, 0)), sideSelectionCount(named));
    assert.equal(
      activeTradeFilterCount(toggleSideOnly(named, 0)),
      activeTradeFilterCount(named),
    );
  });
