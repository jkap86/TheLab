...
/**
 * A calendar date shifted by whole days, both in and out as `YYYY-MM-DD`.
 *
 * Inlined here rather than imported from a date-range module: the seek is the
 * only reader in this app, and it needs exactly one of that module's six
 * helpers. Parsed and formatted as UTC so the arithmetic cannot be moved by the
 * reader's own offset — what the string names is a calendar day, and the zone
 * that day is *read* in is decided by `tradeSeekBounds` below.
 */
...
export const TRADE_CIRCLES: {
  ...
  },
  {
    value: "all",
    label: "Every league",
    note: "Every league this app has stored.",
  },
];
...
export type TradeSideFilter = {
   ...
   * other pick, no FAAB.
   *
   * **A claim about one bay rather than a mode over the selection**, which is
   * what separates it from {@link TradeFilters.match}: `all` and `any` say how
   * the tokens in a bay combine, this says what the bay leaves out. A reader
   * asking "what did jkap give up in the trades where Nabers was *all* he gave"
   * is making that claim about one side and nothing at all about the other, and
   * ticking both bays is the 1-for-1.
   *
   * **It narrows nothing on a bay holding no asset**, so the control is drawn
   * unpressable there and the request leaves it off — "this manager received
   * nothing" is a different question, and the server drops the flag for the same
   * reason.
   */
  only: boolean;
};
 ...
 *
 * **`only` is deliberately not read here.** It is a modifier over the assets a
 * bay names, the way {@link TradeFilters.match} is, so a bay carrying nothing
 * but a raised `only` narrows nothing and has to stay empty — otherwise a flag
 * left behind by a cleared bay would keep a side in the query.
 */
...
/**
 * Raise or lower a bay's "and nothing else" claim.
 *
 * **It does not touch the other bay**, which is the one thing it does
 * differently from {@link toggleSideAsset} and {@link setSideManager}: those two
 * move a token because naming it twice is unsatisfiable, where two bays both
 * claiming exhaustiveness is the perfectly good question "was this trade
 * exactly these for those".
 */
export function toggleSideOnly(
  filters: TradeFilters,
  index: SideIndex,
): TradeFilters {
  const side = filters.sides[index];
  return withSide(filters, index, { ...side, only: !side.only });
}
 ...
 *
 * **`only` is not counted either, for a different reason**: it is a modifier
 * over things already in this number rather than a thing of its own, and a bay
 * can never carry it without also carrying the asset that made it pressable —
 * so counting it would report two narrowings where a reader named one player.
 */
 ...
 * very different lists, and it is set behind a press in the search panel. A
 * bay's `only` is the same case one step further in: the bay itself says it, but
 * the bay is what this line stands in for while the search is shut.
 *
...
export function tradeFilterSummary(
  ...
  return filters.sides
    .map((side, index) => {
      ...
      // Between the label and the assets, where it reads in both directions:
      // "jkap got only Nabers" on his own bay, "jkap gave only Nabers" on the
      // one opposite him — which is the same claim about the same side, said
      // from the two ends `sideLabel` already speaks from.
      const label = `${sideLabel(filters, index as SideIndex, names)}${
        side.only ? " only" : ""
      }`;
      return `${label} ${assets}`;
    })
