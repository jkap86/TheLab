...
/**
 * What a traded pick is worth on the ADP scale, or which of the five ways the
 * answer ran out.
 *
...
export function pickAdpValue({
  ...
  if (!Number.isInteger(pick.round) || pick.round < 1) {
    return { value: null, reason: "bad-pick" };
  }
  ...
  if (rung.startupAdp === null) return { value: null, reason: "no-startup" };
  ...
  return {
    // The startup average, never the rookie-draft one — this is the line the
    // whole module exists to draw, and it is one character from being wrong.
    value: Math.round(adpValue(rung.startupAdp, pool, steepness) * discount.factor),
    player: rung.name,
