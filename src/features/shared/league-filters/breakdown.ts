 ...
 *
 * A pass per row would be four walks over the same list building four
 * intermediate arrays that are immediately reduced to a length — and this
 * re-runs on every keystroke in the rules editor, since the rows describe the
 * *matched* list and the matched list moves as you type. The predicates are
 * resolved once, outside the loop, for the same reason.
 */
