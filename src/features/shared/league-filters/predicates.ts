...
import { SETTING_KEY_BY_KEY, SLOT_GROUP_BY_KEY, TEAMS_KEY } from "./defaults.ts";
 ...
 * says about a selection, because this is the half that has to be *right*: it
 * decides which leagues are on screen, and every rule in it is a quirk somebody
 * was caught by once.
 *
 * The `ManagerLeague` import is type-only, so it is erased and the alias costs
 * nothing — which is what lets this module resolve under Node's test runner
 * despite naming `@/shared/contract`.
 */
 ...
 * reporting that a half-PPR league doesn't pay 0.5. Every comparison is made
 * with a tolerance far below any rate a league actually uses.
 */
const EPSILON = 1e-9;
...
export function slotCount(league: ManagerLeague, key: string): number | null {
  ...
  // rule on every keystroke in a rule's number field, and the array the filter
 ...
 * {@link slotCount} keeps null for. And an **absent key is read per key**:
 * Sleeper omits what a league doesn't set, so a count or a flag is a real 0 (the
 * rule {@link scoringValue} follows) while a week has no zero on its scale and
 * is unknown.
 */
 ...
 * a menu and an is / is not, rather than a bare number field — and a row that
 ...
 *
 * A structural parameter rather than `ManagerLeague`, so anything carrying the
 * blob can ask the question without being one.
 */
...
/**
 * A league's Sleeper type as a number: 0 redraft, 1 keeper, 2 dynasty, 3
 * chopped.
 *
 * Sleeper omits `type` for standard redraft leagues, so a missing value is 0 —
 * the same assumption `LEAGUE_TYPE_SQL` makes in `shared/manager/queries`, and
 * the reason this is a function rather than a field read at each call site: a
 * second copy of the fallback is a second chance to forget it.
 */
 ...
 *
 * Ordered cheapest-first: the two fixed filters are field reads and a settings
 * rule is one property read, where the other two walk `roster_positions` and
 * `scoring_settings` — so a league rejected on its type never touches its
 * lineup.
 *
 * **Every rule narrows** — the lists are an AND, like the fixed filters above
 * them. An OR would need a rule to say which group it joins, and "dynasty
 * leagues that start two QBs" is the question people actually arrive with.
 */
export function matchesFilters(
  league: ManagerLeague,
  filters: LeagueFilters,
): boolean {
  if (filters.type !== "all" && leagueType(league) !== Number(filters.type)) {
    return false;
  }
