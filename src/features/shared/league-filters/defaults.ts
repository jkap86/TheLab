 ...
 * options, the comparisons, and what a slot or settings rule can name.
 *
 * They live apart from the predicates that read them because the two are read
 * by different callers — a control renders these, a filter pass runs those —
 * and because the slot groups are the one thing here with a runtime dependency
 * (`SLOT_POSITIONS`, `IDP_SLOTS`, `QB_ELIGIBLE_STARTING_SLOTS`), which is worth
 * keeping in the module that actually needs it rather than in the one
 * everything imports.
 *
 * **The two slot-vocabulary imports come in relatively with an explicit `.ts`**,
 * which is this repo's documented exception to the import-the-barrel rule:
 * `npm test` runs under Node's type stripper, which resolves the file it is
 * given and knows nothing of the `@/*` aliases — and the `ktc` barrel is
 * server-only besides, since its sync half drags `pg` in. Neither of these two
 * files does; both are pure and free of runtime imports of their own.
 */
 ...
 * vocabulary is read in two places — the dialog's rails and `filterSummary`,
 * which names the active selection outside it. A modal hides its own state, so
 * the words on the page have to come from the same table as the words in the
 ...
 *
 * `activeFilters` and `activeFilterCount` walk this rather than naming each
 * field, so a filter added here is counted and summarised without a second and
 * third edit.
 */
 ...
 * multi-position, no quarterback, no individual defenders. Leagues spell the
 * same idea `FLEX`, `WRRB_FLEX` or `REC_FLEX`, so counting them as one group is
 * what makes "leagues that start three flexes" a question you can ask at all.
 */
 ...
 * `IDP` is `IDP_SLOTS` — whose doc comment names these filters as its reason for
 * existing apart from `DEFENSIVE_SLOTS` — so a new flex counts here the moment
 ...
 *
 * Named rather than spelled at each place that reads it — the predicate, the
 * menu, the presets.
 */
 ...
 * has to stay typeable, and a menu that swallowed the weeks would only offer
 * the eighteen someone thought of. So the row keeps its number field and
 * reaches the sentinel with a key of its own — see {@link settingSentinel}.
 */
 ...
 * with none.
 *
 * The row draws it as a key beside the number field rather than as a menu
 * entry: lit, it *is* the value and the number field stands down; unlit, it is
 * the way in. Both halves are needed because {@link settingValue} reads the
 * sentinel as an absence, so a reader who cannot press it by name cannot ask
 * about it at all.
 */
