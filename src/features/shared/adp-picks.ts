 ...
 *
 * **KTC is read here for two things and drawn for neither.** It supplies the
 * future-season discount ({@link pickAdpStand}) and, through the seasons its
 * board still prices, which future picks there are rows for at all — both facts
 * about the *board* rather than numbers a reader sees. The pick's own KTC price
 * used to ride on the row for a column of its own; that column is the auction
 * pair now, so what is left is the arithmetic that was always this module's.
 *
