/**
 * The `/api/user/[username]/leagues` NDJSON stream, decoded into something a
 * query can resolve with — and the revision dependent reads follow.
 *
 * It was `features/manager/query-fns.ts`, and it is here for the reason
 * `manager-query.ts` and `league-query.ts` are: a second tool reads this stream.
 * The pick tracker's league picker and the lineup checker's whole league list
 * both come off it, and while this decoder lived inside the manager tool the
 * only way for them to read it was a hand-rolled `useEffect` of their own — so
 * the app had two decoders of one protocol and, worse, one of them cached
 * nothing at all. `features/manager/query-fns.ts` re-exports every name under
 * the mover's usual habit, so this feature's own consumers and its tests keep
 * one import site.
 *
 * The one thing worth understanding is why the fetcher takes a `publish`
 * callback rather than simply resolving. The stream is stale-while-revalidate —
 * a cached payload, then a refreshed one over the same connection — so a query
 * that resolved only at the end would sit on a loading screen through a sync the
 * server had already half-answered. Every state it reaches is published as it is
 * learned and the *last* of them is what it resolves with, which is what puts
 * the cached leagues on screen at the first message rather than at the last.
 *
 * Pure apart from this app's own `fetch` wrapper, and imported relatively with a
 * `.ts` extension by the tests that drive it.
 */
...
export type ManagerLeaguesData = {
   ...
   */
  result: LeaguesResultMessage | null;
  /** Per-league sync progress while a sync is running, else null. */
  progress: LeaguesProgressMessage | null;
...
): data is ManagerLeaguesData & { result: LeaguesResultMessage } =>
  Boolean(data?.result);
