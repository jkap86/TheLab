...
// something on the device: the resolved Sleeper account (`account.ts`) and the
// stat-column selections (`use-persisted-columns.ts`). Both were the same twenty
// lines — an external store over `localStorage`, read through
// `useSyncExternalStore` — and the rules those lines encode are subtle enough
// that a second copy is a second chance to get one of them wrong.
//
