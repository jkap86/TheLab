 ...
 * `src/instrumentation.ts` from serving requests against a schema it cannot
 * vouch for.
 */
export async function runMigrations(): Promise<void> {
  ...
  const applied = await runner({
    ...
    // with the same TLS settings as the runtime pool — managed providers
    // require SSL, which a plain connection string omits.
