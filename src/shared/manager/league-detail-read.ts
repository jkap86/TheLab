 ...
 * key: a league nobody has opened all window, opened in two tabs at once, is
 * **one** computation and not two — and the four routes above, fired together by
 * one panel, coalesce onto that same one. A rejection is never stored, so a
 * database blip costs the request that met it and nothing after.
 *
 * Per process, like every cache here. Postgres stays the source of truth and the
 * TTL is inside the crawler's own, so a second instance costs one extra read of
 * an answer that was about to expire.
 */
 ...
 *
 * **The one thing a TTL alone cannot get right here, and therefore the thing
 * that decides how long the TTL may be.** A reader who presses the lineup
 * checker's sync key is told Sleeper has confirmed their change, and the panel
 * refetches — into a cache entry written before the sync ran, for the rest of
 * the window. That is the sync key reporting success and showing the old lineup,
 * which is worse than not having the key. With the press covered here, the TTL
 * only has to be right for writes nobody asked for.
 *
 ...
 * has: a worker dyno's write is visible on the web dyno within the TTL, and the
 * interactive path — the sync route and the reader are on the same web process —
 * is exact.
 */
