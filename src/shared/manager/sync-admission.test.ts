...
describe("managerSyncConcurrency", () => {
  ...
  test("the clamp holds however much is asked for", () => {
    // The property rather than the table: whatever is asked for, manager syncs
    // never claim more of the pool than a single request may hold, and never so
    // little that the admission admits nobody.
