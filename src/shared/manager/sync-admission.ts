 ...
 *
 * Pure: the limiter is a counting semaphore and the environment arrives as an
 * argument, imported relatively with a `.ts` extension so Node's test runner
 * can resolve the chain. The bound is therefore asserted as a *count* of
...
/**
 * The most manager syncs one process runs at once, however it is asked.
 *
...
export type ManagerSyncLimit = {
  /** The bound to use: the request clamped, or the default. */
  limit: number;
...
/**
 * Read {@link MANAGER_SYNC_LIMIT_VAR}.
 *
 * It *requests* a bound and cannot raise one: the number is clamped to
 * {@link DEFAULT_MANAGER_SYNC_LIMIT}, because a knob that can be set to the
 * pool size is the failure this bound exists for reached through the variable
 * meant to prevent it. Junk, zero, a negative and a decimal all fall back
 * rather than failing the boot — a typo in a dashboard should not be why a
 * manager page stops answering.
 */
export function managerSyncLimit(
  env: Record<string, string | undefined> = process.env,
): ManagerSyncLimit {
  const parsed = Number(env[MANAGER_SYNC_LIMIT_VAR]?.trim());
  const requested = Number.isInteger(parsed) && parsed > 0 ? parsed : null;
  const ceiling = DEFAULT_MANAGER_SYNC_LIMIT;
...
/** How many manager syncs one process may run at once. */
export function managerSyncConcurrency(
  env: Record<string, string | undefined> = process.env,
): number {
  return managerSyncLimit(env).limit;
}
 ...
 */
export function clampNotice(name: string, limit: ManagerSyncLimit): string | null {
