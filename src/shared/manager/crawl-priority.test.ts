...
import { NEVER_REFRESHED_AT } from "./sync-freshness.ts";
...
describe("isLeagueRefreshDue", () => {
  test("a league inside its freshness window is not work", () => {
    assert.equal(
      isLeagueRefreshDue(league({ updatedAt: now - ttlMs / 2 }), clock),
      false,
    );
  });

  test("a stale league nobody has tried is", () => {
    assert.equal(
      isLeagueRefreshDue(
        league({ updatedAt: now - ttlMs * 3, syncAttemptAt: null }),
        clock,
      ),
      true,
    );
  ...
  test("a stale league tried a moment ago is not — the retry throttle", () => {
    // The hot loop this closes. A league that cannot sync leaves `updated_at`
    // where it was, so it is stale by definition and was due on every tick;
    // alone in the starved tier it took a batch slot and a Sleeper fan-out every
    // minute, forever.
    assert.equal(
      isLeagueRefreshDue(
        league({ updatedAt: now - ttlMs * 100, syncAttemptAt: now - 1_000 }),
        clock,
      ),
      false,
    );
  ...
  test("and is again once a whole TTL has passed since that try", () => {
    assert.equal(
      isLeagueRefreshDue(
        league({ updatedAt: now - ttlMs * 100, syncAttemptAt: now - ttlMs - 1 }),
        clock,
      ),
      true,
    );
  ...
  test("the ordinary case is untouched: both clocks turn over together", () => {
    // A league claimed at T and refreshed successfully carries T in both
    // columns, so the throttle can never delay a healthy rotation — which is
    // what makes it safe to add at all.
    const refreshedAt = now - ttlMs - 1;
    assert.equal(
      isLeagueRefreshDue(
        league({ updatedAt: refreshedAt, syncAttemptAt: refreshedAt }),
        clock,
      ),
      true,
    );
...
describe("a league whose graph has never been persisted", () => {
  /** What `persistLeagueGraph` writes for a brand-new partial graph. */
  const neverSynced = (over: Partial<CrawlCandidate> = {}) =>
    league({ updatedAt: NEVER_REFRESHED_AT, syncAttemptAt: null, ...over });

  test("is eligible rather than fresh", () => {
    // The bug: the row took `updated_at`'s `DEFAULT now()`, so the very first
    // thing that happened to a league with no graph at all was a full freshness
    // TTL of quiet.
    assert.equal(isLeagueRefreshDue(neverSynced(), clock), true);
  });

  test("and is starved, so it goes before anything merely stale", () => {
    assert.equal(
      leagueRefreshPriority(neverSynced(), clock),
      CRAWL_PRIORITY.starved,
    );
  });

  test("but cannot monopolise the queue once it has been tried", () => {
    // Retry fairness, and the reason the sentinel is affordable. A partial
    // league attempted this tick is out of the running until a TTL has passed;
    // an equally stale league nobody has tried is the work this tick.
    const attempted = neverSynced({ syncAttemptAt: now - 1_000 });
    const untried = league({ updatedAt: now - ttlMs * 2, syncAttemptAt: null });

    assert.equal(isLeagueRefreshDue(attempted, clock), false);
    assert.equal(isLeagueRefreshDue(untried, clock), true);
  });

  test("and sorts behind an untried peer inside its own tier", () => {
    // Same tier, so the tiebreaker decides — and it is the attempt, so a league
    // that keeps failing rotates to the back rather than holding the head.
    const failing = { id: "failing", ...neverSynced({ syncAttemptAt: now - ttlMs - 1 }) };
    const untried = { id: "untried", ...neverSynced() };
    assert.deepEqual(
      [failing, untried]
...
describe("staleLeagueClaimSql", () => {
  ...
  test("the retry throttle is in the WHERE, on the attempt column", () => {
    // The other half of `isLeagueRefreshDue`, and a matched pair with no
    // compiler link: freshness says whether work is needed, the attempt says how
    // often it may be tried, and dropping this half puts a permanently-failing
    // league back in every tick's batch.
    assert.match(
      sql,
      /sync_attempt_at IS NULL\s+OR sync_attempt_at < now\(\) - \$2::interval/,
    );
    // The freshness interval read twice rather than a fourth interval added, so
    // the two questions cannot drift apart.
    assert.equal(sql.match(/\$2::interval/g)?.length, 2);
  });
