 ...
 *
 * Sleeper locks a starter at kickoff. The stored fallback can't say when that
 * was — `projections.game_date` is a `DATE`, so on its own a 1pm game reads as
 * movable until the date rolls over in ET — but the kickoff ordering already
 * reads the schedule's `start_time` per team, and the same instants answer
 * this to the minute.
 *
...
export type LockInputs = {
  ...
  /**
   * Players whose game *date* has passed — `listLineupWeekStats`' own flag,
