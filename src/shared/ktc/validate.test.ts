...
test("a complete board is accepted", () => {
  const result = validateKtcBoard("dynasty", board(520), 515);
...
test("an empty response is rejected", () => {
  const result = validateKtcBoard("dynasty", [], 500);
...
test("a small non-empty fragment is rejected — the case that nulls the board", () => {
  const result = validateKtcBoard("dynasty", board(17), 500);
  assert.equal(result.ok, false);
...
test("the floors are per format — a full redraft board clears its own, not dynasty's", () => {
  // ~250 entries would be a fragment of the dynasty board but is within an
  // off-season trim of the ~370-entry redraft one.
  assert.ok(KTC_MIN_PLAYERS.redraft < 250 && 250 < KTC_MIN_PLAYERS.dynasty);
  assert.equal(validateKtcBoard("redraft", board(250), 0).ok, true);
  assert.equal(validateKtcBoard("dynasty", board(250), 0).ok, false);
});
...
test("a duplicate-heavy response is rejected", () => {
  ...
  const result = validateKtcBoard("dynasty", duplicates, 500);
  assert.equal(result.ok, false);
...
test("a handful of duplicates in a full board is tolerated and deduplicated", () => {
  ...
  const result = validateKtcBoard("dynasty", withDupes, 515);
  assert.equal(result.ok, true);
  assert.equal(result.ok === true && result.players.length, 520);
...
test("a moderate legitimate change is accepted", () => {
  ...
  assert.equal(validateKtcBoard("dynasty", board(492), 500).ok, true);
  assert.equal(validateKtcBoard("dynasty", board(530), 500).ok, true);
});

test("an excessive shrink is rejected even above the absolute minimum", () => {
  const shrunk = Math.floor(1000 * (1 - KTC_MAX_SHRINK)) - 50;
  assert.ok(
    shrunk > KTC_MIN_PLAYERS.dynasty,
    "the floor must not be what rejects this",
  );
  const result = validateKtcBoard("dynasty", board(shrunk), 1000);
  assert.equal(result.ok, false);
...
test("the first sync has nothing to shrink against — only the floor applies", () => {
  const floor = KTC_MIN_PLAYERS.dynasty;
  assert.equal(validateKtcBoard("dynasty", board(floor), 0).ok, true);
  assert.equal(validateKtcBoard("dynasty", board(floor - 1), 0).ok, false);
});
...
test("entries missing an id, a name or any price are dropped as invalid", () => {
  ...
  const result = validateKtcBoard("dynasty", [...board(400, 100), ...junk], 400);
  assert.equal(result.ok, true);
...
test("a player priced on only one board still counts", () => {
  ...
  assert.equal(validateKtcBoard("dynasty", oneSided, 400).ok, true);
});
