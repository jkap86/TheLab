 ...
 *
 * **The fold is per format, and it has to be.** `ktc_values` is keyed
 * `(format, ktc_id)` here where TheLabX keys it on `ktc_id` alone, so the same
 * Sleeper player has a dynasty row *and* a redraft row and they are two
 * markets rather than two readings of one. Folding across them would answer
 * with whichever market happened to price him higher; `./queries` therefore
 * selects one format and hands only those rows here.
 *
