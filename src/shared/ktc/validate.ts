import type { KtcFormat, KtcPlayer } from "./types.ts";
 ...
 * in the response has their values nulled, because KTC's boards churn and a
 ...
 *
 * The naive guard is `players.length > 0`, which every one of those cases
 ...
 * cycle; nulling the board costs everything downstream until someone notices.
 *
 * Pure and tested, like its neighbour `parse`.
 */

/**
 * Fewest valid players a response must carry to be treated as a board, per
 * format — the two boards are different sizes, so one floor cannot serve both.
 * KTC prices ~500 dynasty entries but only ~370 redraft ones (no picks, and
 * kickers and defenses instead), and 300 would sit close enough under 370 that
 * an ordinary off-season trim could wedge the redraft sync. Each floor is set
 * well under its board's true size rather than near it: this is what catches a
 * fragment, and the shrink check below is what catches a response that is
 * merely *short*.
 */
export const KTC_MIN_PLAYERS: Record<KtcFormat, number> = {
  dynasty: 300,
  redraft: 200,
};
...
/**
 * Validate a scraped board against the last stored one **for the same format**.
 *
 * `previous` is how many of this format's players currently carry a price —
 * per-format on purpose, since judging a first redraft scrape against the 500
 * dynasty rows already stored would read as a 26% shrink and refuse a perfectly
 * good board. **Zero means first sync** — there is nothing to shrink against,
 * so only the absolute floor applies; that is what lets an empty database
 * bootstrap without an operator having to disable the guard.
 */
export function validateKtcBoard(
  ...
  if (unique.length < KTC_MIN_PLAYERS[format]) {
    return {
      ok: false,
      reason:
        `only ${unique.length} valid player(s), below the ` +
        `${KTC_MIN_PLAYERS[format]} minimum for the ${format} board`,
      valid: unique.length,
      rejected,
      previous,
    };
