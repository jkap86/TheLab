...
describe("parseTradeQuery", () => {
  ...
  test("a bay can claim it took nothing else", () => {
    assert.equal(parse("s1players=p1&s1only=1").sides[0].only, true);
    assert.equal(parse("s1players=p1&s1only=true").sides[0].only, true);
    // Absent means *off* here, which is why this reads `booleanFlag` and not
    // `booleanFilter` — for a population filter absence means "don't filter".
    assert.equal(parse("s1players=p1").sides[0].only, false);
    // And junk falls back to the neutral form, like every other field.
    assert.equal(parse("s1players=p1&s1only=maybe").sides[0].only, false);
    assert.equal(parse("s1players=p1&s1only=0").sides[0].only, false);
  });

  test("a bay naming no asset makes no such claim, whatever it sent", () => {
    ...
    // nothing.
    assert.equal(parse("s1manager=u1&s1only=1").sides[0].only, false);
    // A bay holding *only* the flag is still an empty bay, so it is dropped.
    assert.deepEqual(parse("s1only=1").sides, []);
    assert.equal(isUnnarrowed(parse("s1only=1")), true);
  });

  test("the claim is per bay, so one can make it and the other not", () => {
    const sides = parse("s1players=p1&s1only=1&s2players=p2").sides;
    assert.equal(sides[0].only, true);
    assert.equal(sides[1].only, false);
  });
  ...
  test("a circle needs an account, and an unknown one is not a circle", () => {
    ...
    assert.equal(
      parse("circle=leaguemate-leagues&user=u1").circle,
      "leaguemate-leagues",
    );

    // Nobody at the centre of it, so there is no circle to draw — and forcing
    // it open rather than 400ing follows the rule the rest of this parser does:
    // the neutral form of a narrowing is not narrowing.
    for (const bad of [
      "circle=mine",
      "circle=mine&user=",
      "circle=mine&user=%20",
  ...
  test("the limit is clamped rather than rejected", () => {
    ...
    assert.equal(
      parse(`limit=${MAX_TRADE_PAGE_SIZE + 1}`).limit,
      DEFAULT_TRADE_PAGE_SIZE,
    );
  ...
  test("the cursor is carried through opaque, and absent is a first page", () => {
    ...
    // same hundred trades appended forever rather than an error.
    assert.equal(
      parse("cursor=eyJhdCI6MSwiaWQiOiJ0OSJ9").cursor,
      "eyJhdCI6MSwiaWQiOiJ0OSJ9",
    );
    assert.equal(parse("").cursor, null);
  });

  test("a cursor is not a narrowing, so a later page asks the same question", () => {
...
describe("what counts as narrowed", () => {
  test("any filter at all takes the query off the unnarrowed board", () => {
    for (const query of [
      "leagues=a",
      "xleagues=a",
      "from=1",
    ...
    // A user with no circle narrows nothing.
...
describe("facetsQuery", () => {
  test("strips the selection and keeps everything the selection is made against", () => {
    // A menu counted over its own selection collapses to that selection the
    // moment you make one, and cannot be widened without being cleared first.
    const query = parse(
      "leagues=a,b&from=1&to=2&s1players=p1&s2manager=u2&circle=mine&user=u1",
    );
    const facets = facetsQuery(query);
    assert.deepEqual(facets.sides, []);
    assert.deepEqual(facets.leagues, ["a", "b"]);
    assert.equal(facets.from, 1);
    assert.equal(facets.to, 2);
    assert.equal(facets.circle, "mine");
    assert.equal(facets.user, "u1");
  });

  test("differs from leagueScopeQuery in keeping the window", () => {
    // The two both drop `sides`, and the window is the whole difference: the
    // headline's denominator is the league scope, where a menu describes the
    // window the reader is looking at.
    const query = parse("from=1&to=2&s1players=p1");
    assert.equal(facetsQuery(query).from, 1);
    assert.equal(leagueScopeQuery(query).from, null);
  });
});
