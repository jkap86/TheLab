 ...
 * built from.
 *
 * **This is the one filter the client cannot resolve for itself, which is why it
 * crosses the wire as a word rather than as a list.** The league rules go the
 * other way — the browser already holds every league of the season for the
 * filter dialog's counts, so it evaluates the rules and sends the answer. A
 * circle's answer is *which leagues a manager fielded a team in* and *who shares
 * one with them*, and a browser holding that would have had to be told it first.
 * So the account id and the circle are what travel, and this resolves them.
 *
 ...
 * same reading `rosterOwners` takes of the same tables: a leaguemate added in
 * the meantime costs a reader their trades for a few minutes rather than being
 ...
 * board for that is empty rather than every trade ever crawled. The page says as
 * much in its own empty state, which is where a reader can act on it.
 */
