 ...
 *
 * **The per-league refresh press is the exception, and for it the cache is the
 * whole problem.** A reader changed a lineup in Sleeper's own app moments ago
 * and pressed a key that says *re-read this league*. A cached answer makes that
 * key look broken while the app behaves exactly as written — the one failure
 * this feature cannot have, because the key exists to be believed.
 *
 ...
 *
 * Minted **once per press and shared by every request that press makes**, so a
 * league's ~11 collections are all read from one moment rather than from eleven.
 * A per-request token would also work as a cache bust and would be worse: it
 * would make the graph a mosaic of instants for no gain.
 *
 ...
 * optional token, and only {@link refreshLeague} ever mints one.
 *
