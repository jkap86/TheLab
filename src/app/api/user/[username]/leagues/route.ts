...
export async function GET(
  ...
  // Three states, not two: `null` is "not asked" and is the only one that may
  // be filled from the resolver. An explicitly requested season is the caller's
  // answer and never goes through it, and an unparseable one is refused rather
  // than quietly becoming the current season.
  ...
  // here therefore can't be a message on the stream.
  ...
  const gate = managerSyncGate(syncState, {
    now,
    requestedAt: now,
    // TheLabX has `?refresh=1` here, honoured only for an internal caller — an
    // operator knob for its crawler, which is not ported. Nothing forces past
    // the TTL yet.
    force: false,
   ...
   * deliberately not "a `manager_syncs` row exists": with `synced_at` advancing
   * only on a complete sync, a manager whose graph keeps half-failing would
   * otherwise read as cold forever and take the cold path — a full foreground
   * fan-out — while 97 of their 100 leagues sat in the database.
   */
  ...
  // is what buys the quiet. A caller with nothing to show is never throttled —
  // there is no cache to serve instead, and that path is bounded by the sync
  // admission below and by the per-manager advisory lock inside the sync.
  ...
  // lock inside the sync is neither of them. A *stale* refresh is the same
  // ~11-requests-per-league fan-out holding the same advisory-lock connection as
  // a cold one, so leaving it unreserved would let any number of stale managers
  // run at once — enough to take a ten-connection pool and then stall the
  // persistence they were queued to do.
  //
  // Reserved here, back-to-back with the gate and with no await between them:
  // taken inside the stream instead, two requests in that window both pass the
  // check and both run the full forced sync.
  //
  // `dedupe: hasCache` is the one asymmetry. A cold caller is never refused as a
  // duplicate, because it has nothing to serve instead and needs its own
  // progress stream, and the advisory lock is what keeps those from repeating
  // each other's fan-out.
   ...
   * rest of the run: bounded by `managerSyncAdmission`, never leaked — the
   * `finally` below releases on every path out, and `release` is idempotent —
   * and paid for by the work being finished rather than repeated.
   */
  let closed = false;
  const stream = new ReadableStream<Uint8Array>({
      ...
      try {
        ...
        if (willRefresh) {
          ...
          send({
            ...
            // Three outcomes leave the list below short of final: a sync that
            // lost the lock (the holder is mid-write, so on a first visit this
            // is a fraction of the leagues), a sync that ran and dropped
            // leagues to a Sleeper failure, and a skip that skipped because the
            // last *attempt* is still inside its throttle window. Reported
            // `stale: false` any of them is a partial list wearing the word
            // "final": the client caches it, closes its progress bar and counts
            // a refresh it never saw. It still ships, because a partial list is
            // worth more than an error and the client's next visit brings it
            // back.
    ...
    cancel() {
      // The consumer stopped reading — a closed tab, a navigation, or the hook
      // aborting its fetch. Everything after this point is for a reader who is
      // no longer there, so `send` becomes a no-op and `start`'s `finally` must
      // not try to close a stream that is already gone. The sync itself runs
      // on; see {@link closed} for why.
      closed = true;
    },
  });
