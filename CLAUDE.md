...
- **The seven league filters are three bands, and three of them read something
  ...
  groups them that way rather than stacking seven equal rows. Four things worth
  keeping:
  - **Each borrows the predicate that already owns the question.**
    ...
    superflex here is priced off the superflex board there. `filters.ts` is tested,
    so all three come in relatively with an explicit `.ts` extension.
