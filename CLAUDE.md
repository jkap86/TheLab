...
Under `APP_PROCESS_ROLE=all` — the default, an unset variable, a laptop, and a
deployment that is one Heroku Basic dyno — the crawler and the request handlers
share a process and a **512 MB quota**. A tick is a Sleeper fan-out over a batch
of leagues whose parsed graphs, rosters, transactions and matchups are all in
memory at once, and it is the only workload in that process that can be asked to
wait: a delayed refresh is a fact about freshness, where a request handler that
cannot allocate is a fact about the app. So `shared/manager/crawl-pressure.ts` is
what makes the crawler stand down, and the priority it encodes is one sentence —
**background freshness is opportunistic, user-facing traffic is not.**

**It is the answer to the contention the worker split answers structurally, for
the deployment that has not taken it.** The two are complementary rather than
alternatives: a role of `web` starts no crawler and this never fires, and the
guard rides the crawler into a `worker` process unchanged — where it bounds that
process's own RSS, which is the right unit either way. Two things read
differently there and neither is a fault. The bands are sized for a process that
is *also* holding Next's runtime, so on a worker alone they are conservative
rather than tight, which is the safe direction; and the pool signal stops meaning
"interactive traffic is queued" and starts meaning "this crawler's own batches
are", which still only ever narrows it. What the guard answers that the split
cannot is the single-dyno question — whether one Basic dyno can serve *and* crawl
— and that is the deployment it was written for.
...
number above is a fixture or a synthetic reading. Four things they cannot check
...
or is effectively dead code there; whether one Basic dyno serving requests *and*
crawling stays under 512 MB at all, which is the question this work exists to
answer and which only the first deployment can; and what the bands should be on a
`worker` dyno, where the same crawler runs without Next's runtime beside it and
the numbers above are conservative by an amount nobody has measured.
